...
The following examples demonstrates best practices for required fields.
...
- Always provide instructions that explain required field formatting to users.
  - If using red asterisks to indicate required fields, include instructions that explains the
...
- The default behavior should be to validate required fields on a user action, such as Continue,
  Save, or Submit.
  - If validations are implemented on blur, this should be on done intentionally on specific fields,
...
- Add the `required` attribute to required form fields to indicate that a field must be filled out
  ...
  - The `noValidate` attribute is set on the form to disable the browser's automatic client side
    validation and utilize custom client side validation. Note `noValidate` will also bypass other
    forms of constraint validation (e.g. `type`, `minLength`, etc.)

## Example 1: Required fields using asterisks
...
## Example 2: Required Fields pattern without asterisks
